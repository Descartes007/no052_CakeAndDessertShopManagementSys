...
public class CatalogServlet extends HttpServlet {
	...
	private static final String CATALOGLIST_PATH="cakeManage/catalogList.jsp";
	private static final String CATALOGADD_PATH="cakeManage/catalogAdd.jsp";
	...
	private void catalogBatDel(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(cd.catalogBatDelById(ids)) {
			request.setAttribute("cakeMessage", "分类已批量删除");
			
		}else {
			request.setAttribute("cakeMessage", "分类删除失败");
		}
		//用户删除成功失败都跳转到用户列表页面
